...
3.  **Test-Driven Development, where it applies:** Write unit tests before
    implementing functionality. See "Two kinds of task" below — this project has
    a substantial body of work that unit tests genuinely cannot verify, and
    pretending otherwise produces tests that assert the mock rather than the
    behaviour.
4.  **High Code Coverage:** Aim for >80% coverage **of the pure-logic modules**
    — rate table, budget translation, cost arithmetic, collector, label and
    output formatting. Coverage of agent-integration code is reported but not
    gated; a high number there measures how thoroughly the SDK was mocked.
4a. **Verify SDK behaviour before coding against it:** Any assumption about
    `google-antigravity` is checked against the installed wheel — by
    introspection or a probe run — *before* code depends on it, and the
    verification is recorded with its version. Two decisions in this project
    were already reversed by doing this; both had been reasoned from
...
Before starting any task, classify it. The lifecycle differs.

**Logic tasks** — the collector, the rate table, `budget_for()`, cost
arithmetic, label sanitisation, comment and artifact formatting. Deterministic,
pure, cheap to test exhaustively. These carry the project's actual claims about
money. **Full TDD applies: red, green, refactor, >80% coverage.**

**Integration tasks** — agent configuration, policies and capabilities, prompts
and system instructions, MCP wiring, anything whose correctness depends on how a
model or the SDK behaves. A unit test here asserts the mock. **These are
verified by a probe run or the M5 eval harness instead**, and the task is not
complete until the observed behaviour is recorded in `docs/probe-results.md`
with the SDK version it was observed against.

A task that looks like both is two tasks. Split it, and keep the logic half
testable.
...
3.  **Write Failing Tests (Red Phase)** — *logic tasks*. For an **integration
    task**, replace this step and step 4 with: write the probe that will
    demonstrate the behaviour, run it, and record the observed result before
    building on it. An unverified SDK assumption is marked `# UNVERIFIED
    (<version>):` at the call site, never left silent.
...
6.  **Verify Coverage:** `uv run pytest --cov --cov-report=term-missing`.
    Target: >80% for new **logic** code. Integration code reports coverage
    without a gate. The
